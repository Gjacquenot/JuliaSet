...
def create_one_julias_set(c=complex(0.0, 0.65), colormap='magma', outputname=None, **kwargs):
    getFilename = lambda colormap, c, invert: colormap+ '/res_{0:04d}_{1:04d}{2}.png'.format(int(1000 * c.real), int(1000 * c.imag), '_level' if invert else '')
    if outputname is None:
        outputname = getFilename(colormap, c, False)
    s = kwargs.get('s', 401)
    x = kwargs.get('x', 2.0)
    ...
    if invert:
        outputname = getFilename(colormap, c, True)
        make_image(Z_level, outputname=outputname, colormap=colormap, dpi=s)


def create_several_julias_set(cn, colormap='magma', outputname=None, **kwargs):
    for c in cn:
        create_one_julias_set(c, colormap='magma', outputname=None, **kwargs)
...
def main():
    ...
    pa('-i', '--invert', action='store_true', help='boolean used to display all levels (disable when tiling)')
    pa('-o', '--output', default=None, help='name of the generated file. If not provided, result will display on screen')
    pa('-x', type=float, default=2.0, help='domain size of the fractal')
    ...
    pa('-n', '--number', type=int, help='number of pictures to generate between', default=1)
    ...
    if len(args.k)==1:
        ...
        create_several_julias_set(c=args.k, colormap=args.colormap, outputname=output, s=args.size, x=args.x, invert=args.invert)
...
if __name__ == '__main__':
    #main()
    # for i in range(1):
    #     # create_one_julias_set(c=complex(0.0, 0.01 + i/100.0), colormap='Greys')
    #     create_one_julias_set(c=complex(0.0, 0.01 + i/100.0), colormap='plasma')
    main()
    #create_one_julias_set(c=complex(-1.417022285618, 0.0099534), colormap='plasma', outputname='super')
    #create_one_julias_set(c=complex(0.285, 0.01), colormap='viridis', outputname='super2', s=1001)
